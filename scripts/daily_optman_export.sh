#!/bin/ksh
# daily_backup.sh
# Tars and copies directories/files required for Maestro master.
. /u02/tws/twa_env.sh

set -x
MAESTRO_HOME=/u02/tws/TWS/
MAESTRO_DATA_HOME=/u02/tws/TWSDATA/
OPTMAN=$MAESTRO_HOME/bin/optman
...
#########################
mkdir -p $BACKUP_DIR
TARGET_DIR=$BACKUP_DIR/optman_`date +%y%m%d%H%M`
mkdir $TARGET_DIR
...
############################
$OPTMAN ls > $TARGET_DIR/optman_ls.txt